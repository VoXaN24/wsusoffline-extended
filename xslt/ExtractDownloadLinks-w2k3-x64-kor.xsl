...
<xsl:template match="*">
  <xsl:choose>
    <xsl:when test="name()='FileLocation'">
      <xsl:if test="contains(@Url, 'http://') and (contains(@Url, 'windowsserver2003-') or contains(@Url, 'windows2003-') or contains(@Url, 'windowsxp-')) and contains(@Url, '-x64-') and contains(@Url, '-kor') and contains(@Url, '.exe')">
